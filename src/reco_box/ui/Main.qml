...
ApplicationWindow {
    ...
    header: ToolBar {
        ...
        RowLayout {
            ...
            ToolIconButton { text: "+"; hint: "添加直播间"; tone: "primary"; onClicked: addDialog.open() }
            Rectangle { Layout.preferredWidth: 1; Layout.preferredHeight: 26; color: window.lineColor }
            ToolIconButton {
                text: "▶"; hint: "一键全部开始录屏 / 监控"; tone: "positive"; enabled: roomModel.count > 0
                onClicked: { roomModel.setAllEnabled(true); monitorCoordinator.checkAllNow(); window.operationMessage = "已启用全部直播间并立即检查" }
            }
            ToolIconButton {
                text: "■"; hint: "一键全部暂停录屏 / 监控"; enabled: roomModel.count > 0
                onClicked: { recordingManager.stopAllAndPause(); window.operationMessage = "已请求全部暂停；正在录制的文件会先安全收尾" }
            }
            ToolIconButton { text: "⌫"; hint: "删除全部直播间"; tone: "danger"; enabled: roomModel.count > 0; onClicked: deleteAllDialog.open() }
            ToolIconButton {
                text: "⚙"; hint: "全局设置、录制历史和运行日志"
                onClicked: { historyModel.refresh(); eventLogModel.refresh(); globalDialog.open() }
            }
            Item { Layout.fillWidth: true }
            ComboBox { id: statusFilter; model: ["全部状态", "录制中", "监控中", "未开始"]; Layout.preferredWidth: 128; Layout.preferredHeight: 42; onActivated: roomProxyModel.setStatusFilter(currentText); background: Rectangle { color: window.surfaceSubtle; radius: 10; border.width: statusFilter.activeFocus ? 2 : 1; border.color: statusFilter.activeFocus ? window.focusColor : window.lineColor } }
            ComboBox { id: sortFilter; model: ["默认排序", "名称正序", "名称倒序"]; Layout.preferredWidth: 128; Layout.preferredHeight: 42; onActivated: roomProxyModel.setSortMode(currentText); background: Rectangle { color: window.surfaceSubtle; radius: 10; border.width: sortFilter.activeFocus ? 2 : 1; border.color: sortFilter.activeFocus ? window.focusColor : window.lineColor } }
            TextField { id: searchField; placeholderText: "搜索主播、标题或链接…"; Layout.preferredWidth: 260; Layout.preferredHeight: 42; leftPadding: 16; rightPadding: 14; onTextChanged: roomProxyModel.setSearchText(text); background: Rectangle { color: window.surfaceSubtle; radius: 10; border.width: searchField.activeFocus ? 2 : 1; border.color: searchField.activeFocus ? window.focusColor : window.lineColor } }
        }
    }
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            Label { text: "直播间"; color: window.ink; font.pixelSize: 26; font.weight: Font.DemiBold }
        ...
        GridView {
                    ...
                    ColumnLayout {
                        ...
                        RowLayout {
                            ...
                            Rectangle {
                                ...
                                Label { id: statusLabel; anchors.centerIn: parent; text: roomStatus === "recording" ? "录制中" : roomStatus === "converting" ? "转 MP4" : roomStatus === "checking" || roomStatus === "preparing" ? "检查中" : roomEnabled ? "监控中" : "未开始"; color: roomStatus === "recording" ? window.success : roomEnabled ? window.muted : window.danger; font.pixelSize: 11; font.weight: Font.DemiBold }
                            }
                        }
                        Label { text: roomTitle.length > 0 ? roomTitle : "暂无直播间标题"; color: window.muted; font.pixelSize: 12; elide: Text.ElideRight; Layout.fillWidth: true }
                        Label { text: "分段：" + (segmentEnabled ? "每 " + segmentMinutes + " 分钟" : "关闭") + (convertToMp4 ? " · 完成后转 MP4" : ""); color: window.success; font.pixelSize: 11; font.weight: Font.DemiBold; Layout.fillWidth: true }
                        Rectangle {
                        ...
                        RowLayout {
                            ...
                            AppButton { text: "检查并录制"; tone: "primary"; Layout.fillWidth: true; enabled: roomEnabled && roomStatus !== "recording" && roomStatus !== "converting" && roomStatus !== "preparing"; onClicked: monitorCoordinator.checkNow(roomId) }
                        }
                        RowLayout {
                            Layout.fillWidth: true; spacing: 6
                            AppButton { objectName: "previewButton"; text: "预览"; compact: true; onClicked: { previewController.play(roomId); previewDialog.open() } }
                            AppButton { text: "编辑"; compact: true; enabled: roomStatus !== "recording" && roomStatus !== "converting" && roomStatus !== "preparing"; onClicked: editDialog.openFor(roomId, streamerName, roomTitle, roomUrl, fileName, checkIntervalSeconds, saveRoot, outputFormat, qualityName, recordingLine, segmentEnabled, segmentMinutes, convertToMp4, audioOnly, recordDanmaku) }
                            Item { Layout.fillWidth: true }
                            AppButton { text: "删除"; tone: "danger"; compact: true; enabled: roomStatus !== "recording" && roomStatus !== "converting" && roomStatus !== "preparing"; onClicked: roomModel.removeRoom(roomId) }
                        }
                    }
                }
            }
            Column {
                ...
                Label { anchors.horizontalCenter: parent.horizontalCenter; text: roomModel.count === 0 ? "还没有直播间" : "没有符合当前筛选条件的直播间"; color: window.ink; font.pixelSize: 17; font.weight: Font.DemiBold }
                Label { anchors.horizontalCenter: parent.horizontalCenter; text: roomModel.count === 0 ? "添加公开直播间地址后，Reco Box 会自动开始监控" : "调整状态筛选或搜索关键词后重试"; color: window.muted; font.pixelSize: 12 }
                AppButton { anchors.horizontalCenter: parent.horizontalCenter; visible: roomModel.count === 0; text: "添加直播间"; tone: "primary"; onClicked: addDialog.open() }
            }
        }
    }
    ...
    Dialog {
        id: addDialog
        title: "添加直播间"; modal: true; anchors.centerIn: parent; width: 540
        standardButtons: Dialog.Ok | Dialog.Cancel
        background: Rectangle { color: window.surface; radius: 16; border.width: 1; border.color: window.lineColor }
        onAccepted: { const root = folderField.text.trim().length > 0 ? folderField.text : settingsController.defaultSaveRoot; if (roomModel.addRoom(urlField.text, nameField.text, root)) { urlField.clear(); nameField.clear(); folderField.clear(); window.operationMessage = "直播间添加成功" } }
        ColumnLayout {
            width: parent.width; spacing: 10
            Label { text: "直播间地址" }
            TextField { id: urlField; Layout.fillWidth: true; placeholderText: "粘贴公开直播间链接" }
            Label { text: "主播名字（可稍后自动识别）" }
            TextField { id: nameField; Layout.fillWidth: true; placeholderText: "待识别主播" }
            Label { text: "保存目录" }
            RowLayout { Layout.fillWidth: true; TextField { id: folderField; Layout.fillWidth: true; placeholderText: settingsController.defaultSaveRoot } AppButton { text: "选择目录"; onClicked: { const value = desktopActions.chooseDirectory(folderField.text); if (value.length > 0) folderField.text = value } } }
            Label { text: "格式、画质、检测间隔和分段使用已保存的全局默认设置。"; color: window.muted; font.pixelSize: 11 }
        }
    }
    ...
    Dialog {
        ...
        title: "编辑直播间"; modal: true; anchors.centerIn: parent; width: 680; height: 650
        background: Rectangle { color: window.surface; radius: 16; border.width: 1; border.color: window.lineColor }
        property string roomId: ""
        property string errorText: ""
        function openFor(id, name, title, url, fileName, interval, root, format, quality, line, segmented, minutes, convert, audio, danmaku) {
            roomId = id; editName.text = name; editTitle.text = title; editUrl.text = url; editFileName.text = fileName.length > 0 ? fileName : name; editInterval.text = String(interval); editRoot.text = root
            editFormat.currentIndex = Math.max(0, editFormat.model.indexOf(format)); editQuality.currentIndex = Math.max(0, editQuality.model.indexOf(quality)); editLine.currentIndex = Math.max(0, editLine.model.indexOf(line === "自动" ? "线路1" : line))
        ...
        ColumnLayout {
            ...
            ScrollView {
                ...
                ColumnLayout {
                    ...
                    Label { text: "基础编辑"; color: window.ink; font.pixelSize: 17; font.bold: true }
                    Label { text: "直播间地址" }
                    TextField { id: editUrl; Layout.fillWidth: true }
                    Label { text: "主播名字" }
                    TextField { id: editName; Layout.fillWidth: true }
                    Label { text: "直播间标题" }
                    TextField { id: editTitle; Layout.fillWidth: true; placeholderText: "可以留空，开播后自动更新" }
                    Rectangle { Layout.fillWidth: true; Layout.preferredHeight: 1; color: window.lineColor; Layout.topMargin: 8; Layout.bottomMargin: 8 }
                    Label { text: "录制设置"; color: window.ink; font.pixelSize: 17; font.bold: true }
                    RowLayout {
                        Layout.fillWidth: true
                        ColumnLayout { Layout.fillWidth: true; Label { text: "文件名（不分段时使用）" } TextField { id: editFileName; Layout.fillWidth: true; placeholderText: "留空则使用 1" } }
                        ColumnLayout { Layout.preferredWidth: 190; Label { text: "检测间隔（秒）" } TextField { id: editInterval; Layout.fillWidth: true; validator: IntValidator { bottom: 30; top: 86400 } } }
                    }
                    Label { text: "保存目录" }
                    RowLayout { Layout.fillWidth: true; TextField { id: editRoot; Layout.fillWidth: true } AppButton { text: "选择目录"; onClicked: { const value = desktopActions.chooseDirectory(editRoot.text); if (value.length > 0) editRoot.text = value } } }
                    RowLayout {
                        Layout.fillWidth: true
                        ColumnLayout { Layout.fillWidth: true; Label { text: "录制清晰度" } ComboBox { id: editQuality; model: ["原画", "蓝光", "超清", "高清", "标清", "流畅"]; Layout.fillWidth: true } }
                        ColumnLayout { Layout.fillWidth: true; Label { text: "录制路线" } ComboBox { id: editLine; model: ["线路1", "线路2", "线路3", "线路4", "线路5"]; Layout.fillWidth: true } }
                    }
                    RowLayout {
                        Layout.fillWidth: true
                        ColumnLayout { Layout.fillWidth: true; Label { text: "输出格式" } ComboBox { id: editFormat; model: ["ts", "mp4", "mkv", "flv", "mp3", "m4a"]; Layout.fillWidth: true } }
                        ColumnLayout { Layout.fillWidth: true; Label { text: "分段时长（分钟）" } RowLayout { Layout.fillWidth: true; CheckBox { id: editSegment; text: "启用" } TextField { id: editMinutes; Layout.fillWidth: true; enabled: editSegment.checked; validator: IntValidator { bottom: 1; top: 1440 } } } }
                    }
                    Label { text: "分段文件固定按 1、2、3… 排列，最后一段按实际剩余时长保存。"; color: window.muted; font.pixelSize: 11 }
                    CheckBox { id: editConvert; text: "录制完成后转为 MP4（成功后删除 TS）"; enabled: editFormat.currentText === "ts" && !editAudio.checked }
                    CheckBox { id: editAudio; text: "纯音频模式" }
                    CheckBox { id: editDanmaku; text: "录制弹幕（仅在平台适配完成后生效）" }
                }
            }
            ...
            RowLayout {
                ...
                AppButton { text: "取消"; onClicked: editDialog.close() }
                AppButton {
                    text: "保存"; tone: "primary"
                    onClicked: { const error = roomModel.updateRoom(editDialog.roomId, editName.text, editTitle.text, editUrl.text, editFileName.text, editInterval.text, editRoot.text, editFormat.currentText, editQuality.currentText, editLine.currentText, editSegment.checked, editMinutes.text, editConvert.checked, editAudio.checked, editDanmaku.checked); if (error.length > 0) editDialog.errorText = error; else { editDialog.close(); window.operationMessage = "直播间设置已保存" } }
                }
            }
        }
    }
    ...
    Dialog {
        id: deleteAllDialog
        title: "删除全部直播间"; modal: true; anchors.centerIn: parent; width: 470
        background: Rectangle { color: window.surface; radius: 16; border.width: 1; border.color: window.lineColor }
        property string errorText: ""
        onOpened: errorText = ""
        ColumnLayout {
            width: parent.width; spacing: 14
            Label { text: "确定从 Reco Box 中删除全部直播间吗？\n录制文件和历史记录不会删除。"; wrapMode: Text.Wrap; Layout.fillWidth: true }
            ...
            RowLayout { Layout.fillWidth: true; Item { Layout.fillWidth: true } AppButton { text: "取消"; onClicked: deleteAllDialog.close() } AppButton { text: "确认全部删除"; tone: "dangerSolid"; onClicked: { const error = roomModel.removeAllRooms(); if (error.length > 0) deleteAllDialog.errorText = error; else { deleteAllDialog.close(); window.operationMessage = "已删除全部直播间" } } } }
        }
    }

    Dialog {
        id: globalDialog
        title: "全局设置"; modal: true; anchors.centerIn: parent; width: 940; height: 680
        standardButtons: Dialog.Close
        background: Rectangle { color: window.surface; radius: 16; border.width: 1; border.color: window.lineColor }
        ColumnLayout {
            anchors.fill: parent; spacing: 10
            TabBar { id: globalTabs; Layout.fillWidth: true; TabButton { text: "全局设置" } TabButton { text: "录制历史"; onClicked: historyModel.refresh() } TabButton { text: "运行日志"; onClicked: eventLogModel.refresh() } }
            StackLayout {
                ...
                ScrollView {
                    ...
                    ColumnLayout {
                        ...
                        RowLayout { Layout.fillWidth: true; Label { text: "新直播间默认设置"; color: window.ink; font.pixelSize: 17; font.weight: Font.DemiBold } Item { Layout.fillWidth: true } AppButton { text: "导入旧配置"; onClicked: legacyDialog.open() } }
                        Label { text: "只影响以后新增或导入的直播间；已有直播间在卡片中单独编辑。"; color: window.muted; font.pixelSize: 11 }
                        Label { text: "默认录制目录" }
                        RowLayout { Layout.fillWidth: true; TextField { id: defaultRootField; text: settingsController.defaultSaveRoot; Layout.fillWidth: true } AppButton { text: "选择目录"; onClicked: { const value = desktopActions.chooseDirectory(defaultRootField.text); if (value.length > 0) defaultRootField.text = value } } }
                        RowLayout {
                            Layout.fillWidth: true
                            ColumnLayout { Layout.fillWidth: true; Label { text: "默认格式" } ComboBox { id: defaultFormat; model: ["ts", "mp4", "mkv", "flv", "mp3", "m4a"]; currentIndex: Math.max(0, model.indexOf(settingsController.defaultOutputFormat)); Layout.fillWidth: true } }
                            ColumnLayout { Layout.fillWidth: true; Label { text: "默认画质" } ComboBox { id: defaultQuality; model: ["原画", "蓝光", "超清", "高清", "标清", "流畅"]; currentIndex: Math.max(0, model.indexOf(settingsController.defaultQuality)); Layout.fillWidth: true } }
                            ColumnLayout { Layout.preferredWidth: 170; Label { text: "检测间隔（秒）" } TextField { id: defaultInterval; text: String(settingsController.defaultCheckInterval); validator: IntValidator { bottom: 30; top: 86400 } Layout.fillWidth: true } }
                            ColumnLayout { Layout.preferredWidth: 170; Label { text: "磁盘保护（GB）" } TextField { id: minimumFreeGb; text: String(settingsController.minimumFreeGb); validator: IntValidator { bottom: 1; top: 1024 } Layout.fillWidth: true } }
                        }
                        CheckBox { id: defaultSegment; text: "新直播间默认启用分段"; checked: settingsController.defaultSegmentEnabled; onClicked: window.settingsMessage = "设置已更改，请点击保存设置" }
                        RowLayout { enabled: defaultSegment.checked; Label { text: "每段" } TextField { id: defaultSegmentMinutes; text: String(settingsController.defaultSegmentMinutes); Layout.preferredWidth: 100; validator: IntValidator { bottom: 1; top: 1440 } onTextEdited: window.settingsMessage = "设置已更改，请点击保存设置" } Label { text: "分钟，最后一段按实际时长保存"; color: window.muted } }
                        RowLayout {
                            Layout.fillWidth: true
                            Label { text: "启动后自动监控 · 关闭窗口最小化到托盘 · 不随 Windows 开机启动"; color: window.muted; font.pixelSize: 11 }
                            Item { Layout.fillWidth: true }
                            AppButton { text: "保存设置"; tone: "primary"; onClicked: { const error = settingsController.saveDefaults(defaultRootField.text, defaultFormat.currentText, defaultQuality.currentText, defaultInterval.text, defaultSegment.checked, defaultSegmentMinutes.text, minimumFreeGb.text); window.settingsMessage = error.length > 0 ? error : (defaultSegment.checked ? "已保存并确认：以后新增直播间默认每 " + defaultSegmentMinutes.text + " 分钟分段" : "已保存并确认：以后新增直播间默认不分段") } }
                        }
                        Label { text: window.settingsMessage; color: window.settingsMessage.indexOf("已保存并确认") === 0 ? window.success : window.settingsMessage.indexOf("设置已更改") === 0 ? window.warning : window.danger; font.pixelSize: 11; font.weight: Font.DemiBold }
                    }
                }
                ListView {
                    ...
                    delegate: Rectangle {
                        ...
                        RowLayout {
                            ...
                            Label { text: probe_status === "valid" ? "可播放" : status === "converting" ? "转 MP4 中" : status === "recording" ? "录制中" : status === "completed" ? "已完成" : "失败"; color: probe_status === "valid" || status === "completed" ? window.success : window.ink; font.weight: Font.DemiBold }
                            AppButton { text: "播放"; compact: true; enabled: status !== "recording" && status !== "converting"; onClicked: desktopActions.playRecording(String(session_dirs)) }
                            AppButton { text: "目录"; compact: true; enabled: status !== "recording" && status !== "converting"; onClicked: desktopActions.openRecordingDirectories(String(session_dirs)) }
                        }
                    }
                }
                ColumnLayout {
                    Label { text: "这里只显示脱敏后的状态和错误，不保存完整临时播放地址。"; color: window.muted; font.pixelSize: 11 }
                    ListView {
    ...
    Dialog {
        ...
        MediaPlayer {
            ...
            onErrorOccurred: function(error, errorString) {
                previewController.setPlayerError(errorString.length > 0 ? errorString : "直播流播放失败")
            }
            onMediaStatusChanged: {
                if (mediaStatus === MediaPlayer.InvalidMedia)
                    previewController.setPlayerError("直播流格式无效或播放器无法解码")
            }
        }
        ...
        ColumnLayout {
            ...
            RowLayout { Layout.fillWidth: true; Label { text: previewController.error; color: window.danger; elide: Text.ElideRight; Layout.fillWidth: true } AppButton { text: previewPlayer.playbackState === MediaPlayer.PlayingState ? "暂停" : "播放"; tone: "primary"; enabled: previewController.source.toString().length > 0; onClicked: previewPlayer.playbackState === MediaPlayer.PlayingState ? previewPlayer.pause() : previewPlayer.play() } AppButton { text: "静音"; checkable: true; onToggled: previewPlayer.audioOutput.muted = checked } }
        }
    }

    Dialog {
        id: legacyDialog
        title: "导入 DouyinLiveRecorder 旧配置"; modal: true; anchors.centerIn: parent; width: 640; standardButtons: Dialog.Close
        background: Rectangle { color: window.surface; radius: 16; border.width: 1; border.color: window.lineColor }
        ColumnLayout {
            width: parent.width; spacing: 10
            Label { text: "选择旧程序根目录或其中的 config 文件夹。" }
            RowLayout { Layout.fillWidth: true; TextField { id: legacyFolderField; Layout.fillWidth: true } AppButton { text: "选择文件夹"; onClicked: { const value = desktopActions.chooseDirectory(legacyFolderField.text); if (value.length > 0) legacyFolderField.text = value } } }
            RowLayout { AppButton { text: "1. 预检"; enabled: legacyFolderField.text.length > 0; onClicked: legacyImport.preview(legacyFolderField.text) } AppButton { text: "2. 确认导入"; tone: "primary"; enabled: legacyImport.previewText.indexOf("可导入直播间") >= 0; onClicked: legacyImport.runImport(legacyFolderField.text) } }
            ...
            Label { text: "不会修改旧配置；Cookie、令牌、账号、密码和推送凭据不会导入。"; color: window.muted; font.pixelSize: 11 }
        }
    }
}
