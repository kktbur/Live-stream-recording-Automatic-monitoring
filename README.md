...
> 当前为 `0.1.3` 早期测试版，仅面向 Windows x64。直播平台经常调整网页和接口，因此“已支持”不代表任何直播间都能永久稳定解析。

## 下载与安装

在仓库右侧的 **Releases** 页面下载：

- `RecoBox-Setup-0.1.3.exe`：Windows 标准安装包
- `RecoBox-Setup-0.1.3.exe.sha256.txt`：安装包校验值

安装前建议核对 SHA-256。安装包目前没有商业代码签名，Windows SmartScreen 可能显示“未知发布者”。

安装完成后直接打开 Reco Box 即可。当前解析器的部分 JavaScript 签名流程仍依赖系统中的 Node.js；如果自检提示缺少 Node.js，请安装 Node.js 18 或更高版本后重启软件。
...
要求：Windows x64、Python 3.12、Node.js 18+、PowerShell 7。
