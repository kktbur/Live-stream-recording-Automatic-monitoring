...
| YouTube | 支持 | 匿名解析；页面和 HLS 清单请求已接入第一方 TLS 默认校验 |
...
锁定的上游直播解析器对多数主要平台解析请求仍默认不校验 TLS 证书。这是上游为兼容部分反爬环境保留的设计，可能降低网络传输的抗中间人攻击能力；请仅在可信网络环境中使用。PR-06 已建立 [平台网络安全矩阵](docs/platform-network-security.md)，并将 Reco Box 自己控制的 TwitCasting 匿名请求接入默认校验 TLS 的第一方策略；PR-07 将 Bilibili API 请求迁移到第一方 HTTPX 客户端并默认校验证书；PR-08 又将 YouTube 页面和 HLS 清单请求迁移到第一方 HTTPX 客户端并默认校验证书。其他上游路径仍需逐平台验证，不能把这些步骤理解为所有平台已经完成整改。Reco Box 当前不会把解析到的临时直播地址写入日志。
